...
-- 1b. Tabel Akun (dipakai untuk login — belum ada sebelumnya)
CREATE TABLE Akun (
    id_akun INT PRIMARY KEY AUTO_INCREMENT,
    id_pasien INT NULL,
    no_hp VARCHAR(20) UNIQUE NOT NULL,
    password_hash VARCHAR(255) NOT NULL,
    role ENUM('pasien', 'admin', 'dokter') NOT NULL DEFAULT 'pasien',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (id_pasien) REFERENCES Pasien(id_pasien) ON DELETE SET NULL
);
